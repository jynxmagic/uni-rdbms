...
--director2
INSERT INTO DIRECTOR(DIRECTOR)
    SELECT DISTINCT DIRECTOR2
    FROM OLD_TITLES
    WHERE DIRECTOR2 IS NOT NULL
    AND DIRECTOR2 IN (SELECT PERSON_ID FROM PERSON)
    AND DIRECTOR2 NOT IN (SELECT PERSON_ID FROM DIRECTOR);
COMMIT;

--director3
INSERT INTO DIRECTOR(DIRECTOR)
    SELECT DISTINCT DIRECTOR3
    FROM OLD_TITLES
    WHERE DIRECTOR3 IS NOT NULL
    AND DIRECTOR3 IN (SELECT PERSON_ID FROM PERSON)
    AND DIRECTOR3 NOT IN (SELECT PERSON_ID FROM DIRECTOR);
COMMIT;

--director4
INSERT INTO DIRECTOR(DIRECTOR)
    SELECT DISTINCT DIRECTOR4
    FROM OLD_TITLES
    WHERE DIRECTOR4 IS NOT NULL
    AND DIRECTOR4 IN(SELECT PERSON_ID FROM PERSON)
    AND DIRECTOR4 NOT IN (SELECT PERSON_ID FROM DIRECTOR);
COMMIT;



--verification
SELECT COUNT(director.DIRECTOR) AS "# of directors", count(person.person_id) AS "# of people"
FROM DIRECTOR
LEFT OUTER JOIN PERSON
ON PERSON.PERSON_ID = DIRECTOR.DIRECTOR;
...
INSERT INTO GENRE(GENRE)
    ...
    AND GENRE2 NOT IN (SELECT genre FROM GENRE);
COMMIT; 
...
INSERT INTO GENRE(GENRE)
    ...
    AND GENRE3 NOT IN (SELECT genre FROM GENRE);
COMMIT;
